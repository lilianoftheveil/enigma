...
def densidade_lexical(texto):
    ...
    palavras_conteudo = [palavra for palavra in palavras if palavra not in stopwords_pt]
    
    num_palavras_conteudo = len(palavras_conteudo)
    
    densidade_lexical = round(num_palavras_conteudo / num_tokens if num_tokens > 0 else 0, 4)
    
    resultado_json = {
        'numero_palavras': num_tokens,
        'numero_semantico': num_palavras_conteudo,
        'densidade_lexical': densidade_lexical
    }
