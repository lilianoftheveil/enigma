...
def processar_pron(texto):
    ...
    for word, tag in tagged_words:
        if tag in ['PROPESS', 'PROSUB', 'PROADJ', 'PRON', 'PRO-KS', 'PRO-KS-REL']:
            pronomes.append(word)
    
    contagem_pronomes = Counter(pronomes).most_common(20)
    
    resultado_json = {
        'pronomes': {pronome: contagem for pronome, contagem in contagem_pronomes}
    }
...
resultado = processar_pron(texto)
print(resultado)
