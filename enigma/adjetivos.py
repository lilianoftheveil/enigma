...
def processar_adj(texto):
    ...
    for word, tag in tagged_words:
        if tag == 'ADJ' or tag.startswith('ADJ-'): 
            adjetivos.append(word)
    
    contagem_adjetivos = Counter(adjetivos).most_common(20)
    
    resultado_json = {
        'adjetivos': {adjetivo: contagem for adjetivo, contagem in contagem_adjetivos}
    }
